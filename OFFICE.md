...
| `src/server/roles.ts` | Роли: промпт, модель, изоляция, номер копии пакета |
| `src/server/offices.ts` | Реестр офисов: офис = проект со своей директорией и состоянием |
| `src/server/setup.ts`, `src/server/pickfolder.ts` | Мастер нового офиса: витрина, разбор плана, сборка шагами (корень, папки ролей, найм, направление) с прогрессом; нативный диалог выбора папки на машине сервера — `docs/design/office-setup/spec.md` |
...
| `src/server/limits.ts` | Лимиты плана подписки: проценты и время сброса из событий SDK; отказ по лимиту и ожидание сброса (`limitBlock`) |
...
| `src/server/rules.ts` | Правила офиса: `RULES.md` в корне офиса и в корне репозитория роли, круги (офис → направление → роль), блок в системный промпт, инструменты менеджера — `docs/design/rules/spec.md` |
...
- **Язык кода, комментариев, коммитов и документации — язык реализации из
  настроек офиса** (в этом офисе он русский), а язык отчётов, задач и разговора
  с владельцем — язык общения (тоже русский). Зашитого «по-русски» больше нет:
  оба языка приезжают в системный промпт отдельным блоком, и при расхождении с
  этим файлом сильнее настройка. Тексты интерфейса живут в словарях
  (`src/web/i18n`, `src/server/i18n`) и пишутся на оба языка сразу — язык
  интерфейса переключает пользователь. Комментарий объясняет «почему так», а не
  пересказывает код.
