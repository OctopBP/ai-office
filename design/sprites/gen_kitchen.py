...
# Footprint тумб (спека §6.3, §7): верхняя полоса картинки (0-6 art-px) —
# технологический отступ до столешницы, тот же приём, что и у desk() в
# gen.py, — реальная занятая площадь начинается от столешницы. У
# counter_corner отступ такой же по y, а x/w оставлены на всю картинку: L-форму
# тумбы одним прямоугольником честно не описать, берём её бокс целиком.
gen.SPRITE_FOOTPRINT.update({
    'counter_straight': [0, 0.375, 2.0, 1.0],
    'counter_corner': [0, 0.375, 2.0, 1.625],
    'sink_counter': [0, 0.375, 2.0, 1.0],
    # dining_table (T-91): плоская столешница (0-15 art-px) в этом ракурсе —
    # то же перспективное продолжение поверхности вверх по спрайту, что и
    # монитор у desk(), только без явного отступа для предмета сверху; на
    # полу же реально стоит только нижняя часть — фасад (16-19) и ножки
    # (20-21). С полноразмерным footprint эти два ряда клеток под столом
    # (footprint из 2 строк при нецелой позиции стола) наглухо запечатывали
    # места кухни у северной стороны: вход к ним шёл только через южный ряд,
    # а тот перекрывался столом целиком. Узкий footprint по нижнему краю
    # освобождает соседний ряд клеток и оставляет проход к местам.
    'dining_table': [0, 1.0, 6, 0.375],
})

# Препятствия (спека §7): тумбы и обеденный стол — мебель, через которую
# нельзя пройти. Кофемашина и мелочь на столе (kitchen_mugs/kitchen_snack) не
# блокируют — это декор, лежащий поверх уже блокирующей тумбы/стола.
gen.SPRITE_BLOCKS.update({
    'counter_straight', 'counter_corner', 'sink_counter',
    'dining_table',
})
