...
# сюда не входят.
#
# Стулья (chair) занесены в блокирующие: сам стул физически загораживает
# клетку, но именно этот случай и покрывает правило спеки «слоты всегда
# проходимы» — место посадки за обеденным/переговорным столом считается по
# слоту стола (side/ring), а не по спрайту стула, поэтому клетка, куда
# приходит агент, чтобы сесть, остаётся достижимой независимо от того, что на
# ней нарисован стул.
#
