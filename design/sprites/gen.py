...
ART_PATH = os.path.join(ROOT, 'art.json')
CATALOG = {}  # имя спрайта → размер в тайлах, копится за время работы процесса
# Что генератор про спрайт знает — и только это.
#
# Раньше здесь же лежали footprint, слоты, проходимость и подписи мебели: три
# сотни строк рукописных таблиц внутри рисовалки картинок. Генератор про
# посадочные места ничего не знает и знать не должен — они уехали в пресеты
# (docs/design/office-presets/spec.md). Осталось то, что и правда свойство
# арта: размер в тайлах, посчитанный по пикселям.
#
# Два исключения, и оба не мебель:
#  - тайлы стен: у них нет пресета, а footprint и слой им нужны, и заданы они
#    не поштучно, а одной строкой на весь автотайл;
#  - подписи внешностей агентов: они часть определения самого спрайта
#    (цвет рубашки плюс название), а не отдельная авторская таблица.
SPRITE_FOOTPRINT = {}  # имя спрайта → footprint [x,y,w,h] от якоря, тайлы (только тайлы стен)
SPRITE_LAYER = {}  # имя спрайта → слой для сортировки (только тайлы стен)
SPRITE_LABEL = {}  # имя внешности агента → человекочитаемое русское название
...
# Запуск с --catalog: пересобрать только art.json, картинки не трогать.
...
def save(im, name):
    ...
    # В режиме --catalog картинки не перезаписываются: PIL пишет PNG побайтово
    # иначе, чем в прошлый раз, и в диффе оказывается весь набор спрайтов
    # вместо одной строки.
    if not CATALOG_ONLY:
...
def dump_art():
    """Слить размеры этого запуска в design/sprites/out/art.json.

    Это **не** каталог. Каталог (`catalog.json`) собирается из пресетов
    скриптом `npm run presets:catalog`, и автор у него теперь один; генератор
    отдаёт только то, что посчитал по пикселям, и то, что относится к
    спрайтам без пресета — тайлам стен и внешностям агентов.
    ...
    for name, size in CATALOG.items():
        # Размер трогаем только тогда, когда картинку и правда перерисовали.
        #
        # Он давно не «посчитан по пикселям»: арт предметов рисует
        # `tools/props` в произвольном разрешении (desk.png — 424×332 при
        # логических 2×1.375 тайла), а здесь считается по процедурному холсту
        # этого файла. Для спрайтов, которые gen.py и правда рисует, это одно
        # и то же; для остальных — нет, и перезаписывать их размер по своему
        # холсту значит портить данные кодом, который к этой картинке уже не
        # имеет отношения. В режиме --catalog картинки не трогали — значит и
        # размеру взяться неоткуда, только сверить и сказать вслух.
        was = sprites.get(name, {}).get('size')
        if CATALOG_ONLY and was is not None and was != size:
            print(f'  ! {name}: размер в файле {was}, холст даёт {size} — оставлен файловый')
            size = was
        entry = {'size': size}
    ...
    with open(ART_PATH, 'w') as f:
        ...
        f.write('\n')
    print('art', ART_PATH, len(sprites), 'спрайтов')
    print('каталог собирается отдельно: npm run presets:catalog -- --write')
