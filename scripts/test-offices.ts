...
async function main(): Promise<void> {
  ...
  // 26. Язык интерфейса. Из трёх языков офиса он единственный глобальный:
  //     один на всё приложение, лежит в реестре рядом со списком и не зависит
  //     от того, какой офис сейчас открыт. Проверяется ровно то, на чём он
  //     соврал бы незаметно, — что он общий для всех офисов, что язык общения
  //     офиса его не утаскивает, что он доезжает до диска и переживает
  //     перезапуск и что реестру старше разделения языков миграция проставляет
  //     нынешнюю локаль, а не умолчание.
  ...
  check('о смене узнал и клиент без открытого офиса',
    b.last('ui.language', markUi)?.lang === 'en' && noOffice.last('ui.language')?.lang === 'en');
  ...
  sendSnapshot(snapOne, restarted);
  ...
  const born = createOffice({ name: 'Новый', projectDir: DIR_E, mustExist: true });
  ...
  check('чужой язык интерфейса не принимается и рассылки не вызывает',
    uiLanguage() === 'en' && b.count('ui.language', markUi) === 0);
  ...
  // Перезапуск сервера: реестр читается с диска вторым экземпляром модуля.
  // Адрес через переменную — иначе tsc ищет модуль по строке с хвостом.
  const restartPath = '../src/server/offices.ts?restart=2';
  const restartUi = await import(restartPath) as typeof import('../src/server/offices');
  ...
  const saved = onDisk();
  check('реестр на диске знает все пять офисов, включая скрытый',
    saved.offices.length === 5 && saved.offices.filter((o) => o.hidden).length === 1);
