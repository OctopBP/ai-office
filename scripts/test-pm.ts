...
const SCENARIOS: Scenario[] = [
  ...
  {
    name: 'Отвечает на вопрос, не заводя задач',
    prompt: 'Скажи, что сейчас в работе у команды и кто чем занят? Ничего делать не надо, просто расскажи.',
    checks: [
      { what: 'ответил словами', ok: (r) => r.pmText.trim().length > 20 },
      {
        what: 'не завёл ни одной задачи',
        ok: (r) => r.tasks.length === 0 && !r.toolCalls.some((t) => t.includes('create_task')),
      },
      { what: 'не спрашивал, что бы поручить', ok: (r) => !/поруч|назнач/i.test(r.pmText) || /не (нужно|надо)/i.test(r.pmText) },
    ],
  },
  {
