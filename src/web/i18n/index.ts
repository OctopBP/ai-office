 ...
 * Язык интерфейса.
 *
 * Настоящий язык интерфейса живёт на сервере в реестре офисов — он один на
 * всё приложение, а не на офис, — и приезжает событием `ui.language` (оно же
 * первым ответом на подключение) и полем `uiLanguage` в снимке состояния.
 * Здесь он дублируется в localStorage — не как второй источник правды, а как
 * ответ на вопрос «на каком языке рисовать первый кадр», который задаётся
 * раньше, чем ответил сервер.
 *
 * Языки офиса — общения (`Settings.chatLanguage`) и реализации
 * (`Settings.codeLanguage`) — сюда не приходят вовсе: они про то, на чём
 * говорит команда и на чём написан проект, а не про подписи на экране.
 *
...
/**
 * Запомнить язык интерфейса. Возвращает true, если он изменился, — по этому
