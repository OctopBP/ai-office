...
/**
 * Внешний контур модели окна `design/models/furniture/window.glb` — габариты
 * её bounding box (все меши сцены модели вместе с рамой, а не одно стекло):
 * 1.7997 × 2.5634 × 0.1726 единиц модели. Замерено по самому файлу: min/max
 * аксессоров POSITION с учётом матриц узлов — то же, что даёт
 * `THREE.Box3().setFromObject()` на загруженной сцене.
 *
 * Числа лежат константой, а не берутся замером, потому что `scene3` считает
 * стены синхронно и задолго до того, как GLTF-загрузчик отдаст модель: проём
 * нужен уже в первом кадре, а модель приезжает позже. Чтобы константа не
 * разошлась с файлом, `Window3D.tsx` меряет загруженную модель
 * `Box3.setFromObject` и ругается в консоль, если пропорция уехала.
 */
export const WINDOW_MODEL_SIZE = { w: 1.7997, h: 2.5634, d: 0.1726 };

/** Во сколько раз модель окна выше своей ширины — 1.4243. */
export const WINDOW_ASPECT = WINDOW_MODEL_SIZE.h / WINDOW_MODEL_SIZE.w;

/**
 * Высота проёма, тайлов. Проём режется по внешнему контуру окна, а не по его
 * стеклянной части: раньше высота была зашита числом от стекла (0.6…2.15), и
 * модель, чтобы рама не обрывалась на краю дырки, рисовалась крупнее проёма —
 * рама наполовину уходила в стену и стык было видно. Теперь наоборот: дырка в
 * стене — ровно того размера, что модель вместе с рамой.
 *
 * Ширину проёма задаёт раскладка (окно — клетка стены, тайл в ширину), поэтому
 * от модели берётся пропорция: высота = ширина клетки × `WINDOW_ASPECT`. Так
 * окно в обычной одноклеточной дырке садится вообще без растяжения — масштаб
 * по обеим осям выходит один и тот же. Окно шире одной клетки (несколько
 * оконных клеток подряд) остаётся той же высоты: ряд окон на стене должен быть
 * одной высоты, иначе стена расползается.
 */
const WINDOW_CELL = 1;
export const WINDOW_OPENING_H = WINDOW_CELL * WINDOW_ASPECT;
 ...
 *
 * Середина проёма оставлена там же, где была при T-72 (между 0.6 и 2.15), —
 * менялся размер выреза, а не место окна на стене.
 */
const WINDOW_MID = 1.375;
export const WINDOW_SILL = WINDOW_MID - WINDOW_OPENING_H / 2;
export const WINDOW_HEAD = WINDOW_MID + WINDOW_OPENING_H / 2;
