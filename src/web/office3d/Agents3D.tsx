 ...
 *
 * Рядов ровно три, и появляются они все вместе: профессия, задача (номер
 * плашкой и название), текущая команда. Без задачи остаётся один верхний ряд —
...
function AgentTag({ anchorRef, inst, role, task }: {
  ...
  /**
   * Полное название профессии — то же, чем сотрудник подписан в списке
   * команды: `role.title` из настроек роли, а если роль почему-то не доехала —
   * `inst.label`, подпись, которую офис держит для всех остальных мест.
   ...
   */
  const title = role?.title ?? inst.label;
