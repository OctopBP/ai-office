...
/**
 * Причина падения по лимиту ходов. Лимит наш, а не SDK: без подсказки, где он
 * правится, сообщение выглядит как поломка Claude Code и чинить его нечем.
 */
const MAX_TURNS_HINT =
  'исчерпан лимит ходов задачи — поднимите его в настройках офиса или разбейте задачу на части';
...
function resultReason(msg: Extract<SDKMessage, { type: 'result' }>): string {
  ...
  if (msg.subtype === 'error_max_turns') return MAX_TURNS_HINT;
  if ('result' in msg && typeof msg.result === 'string' && msg.result.trim()) return msg.result;
  return msg.subtype;
}

/**
 * То же для исключения: лимит ходов прилетает и результатом со `subtype`, и
 * брошенной ошибкой SDK («Reached maximum number of turns»), а причина одна.
 */
function humanFailure(message: string): string {
  return /maximum number of turns|error_max_turns/i.test(message) ? MAX_TURNS_HINT : message;
}
    ...
    try {
      ...
      const session = query({
        ...
        options: {
          ...
          settingSources: [],
          sandbox: SANDBOX,
          maxTurns: office.settings.taskMaxTurns ?? undefined,
      ...
      const message = humanFailure((err as Error).message);
      ...
      if (stoppedByUser.delete(task.id)) {
        ...
        // Наработки коммитим и при падении, а не только при ручной остановке.
        // Исполнитель мог сделать почти всё и упереться в лимит на последнем
        // шаге: раньше сделанное оставалось некоммитнутым в worktree и пропадало
        // вместе с ним при перезапуске задачи — от часа работы оставалась строка
        // об ошибке. Перезапуск такую ветку сохранит под другим именем (retryTask).
        office.addLog(inst.id, 'error', `Задача ${task.id} упала: ${message}`);
        const fresh = office.tasks.get(task.id);
        let saved = false;
        if (fresh?.branch) {
          saved = await commitAll(workRoot, `${task.id}: частичная работа (падение)`) === 'committed';
          if (saved) {
            office.addLog(inst.id, 'system', `Наработки упавшей задачи закоммичены в ${fresh.branch}`);
          }
        }
        office.updateTask(task.id, {
          status: 'failed',
          result: `Ошибка: ${message}` + (saved ? `\n\n💾 Сделанное закоммичено в ${fresh?.branch}.` : ''),
          finishedAt: Date.now(),
        });
        office.setState(inst.id, 'failed', 'ошибка');
        notifyPm(
          `[СИСТЕМА] Задача ${task.id} провалилась у ${inst.id}. Ошибка: ${message}\n` +
          (saved
            ? `Сделанное закоммичено в ветку ${fresh?.branch} — перезапуск начнёт с нуля, но эту ветку офис сбережёт.\n`
            : 'Изменений в рабочей копии не осталось.\n') +
          'Реши, перезапускать ли задачу, и скажи об этом пользователю.',
        );
      }
    } finally {
      inst.currentTaskId = null;
