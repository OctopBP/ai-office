 ...
 *
 * Номер идёт без решётки: с ней подпись читалась как код экземпляра
 * (`backend#2`), а код — внутреннее дело офиса, владельцу он не адресован.
 * На экране ту же подпись собирает `src/web/instanceName.ts`; здесь она
 * остаётся для строк, которые пишет сам офис, — чат, лента, совещания.
 */
