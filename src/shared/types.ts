 ...
 */
export type OfficeOp = 'create' | 'switch' | 'rename' | 'remove' | 'open' | 'icon';
 ...
 * не повторяется на сервере и в вебе.
 *
 * Сортируем по времени создания, самый старый сверху. Это единственное
 * свойство офиса, которое не меняется никогда: имя переименовывают,
 * `lastOpenedAt` едет при каждом входе, активность — вообще каждую секунду,
 * и сортировка по любому из них переставляла бы строки под рукой человека.
 * Текущий офис наверх не всплывает: он выделяется только видом.
 *
...
export function compareOffices(
  a: { id: string; createdAt: number }, b: { id: string; createdAt: number },
): number {
