 ...
 * `image` — путь к картинке ОТНОСИТЕЛЬНО директории офиса, её надо запросить
 * у сервера (`/api/office-icon?office=<id>`), из браузера файл с диска не
 * открыть. Относительный путь, а не абсолютный: папку проекта переносят,
 * а иконка должна ехать вместе с ней.
 *
